...
function makeBody(row, actions, actionFunct) {
    ...
    for (var i = 0; i < actions.length; i++) {
        checks += "<td><input type='checkbox' value='' id='"+ actions[i].id_accion + row.id_funcionalidad + i +"' " + checkActionFunct(actionFunct, actions[i].id_accion, row.id_funcionalidad) +" onclick='loadSelectionActionFunct("+ actions[i].id_accion + ", " + row.id_funcionalidad + "," + actions[i].id_accion + row.id_funcionalidad + i +")'><label class='form-check-label' for='flexCheckDefault'></td>";
    }
...
function makeHead(row, $i) {
    ...
    var rowTable =
        ...
        '<img class="iconoOrdenar" src="Resources/ordenacion.png" alt="Ordenar" /> <span class="tooltiptext ORDENAR"></span>'+
        '</div></th>';
