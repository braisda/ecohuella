		...
		switch($action){
			case 'search':
				...
				foreach ($elements['resource'] as $elem) {
					...
					} else if ($elem['borrado_logico'] === 0 && rolUserSystem == '2' && $elem['id_rol']['id_rol'] != '1') {
                        array_push($users, $elem);
					}  else if ($elem['borrado_logico'] === 0 && rolUserSystem == '3' && $elem['id_rol']['id_rol'] == '3' ) {
                        array_push($users, $elem);
