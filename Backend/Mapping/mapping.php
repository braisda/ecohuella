...
class mapping extends MappingBase
{
    ...
    function SEARCH_GENERICO($table, $arrayDataValue, $foraneas, $start, $pageRaw, $orden, $tipoOrden, $id)
    {
        ...
        if (!empty($this->feedback['resource']) && !empty($foraneas)) {
            foreach ($foraneas as $key => $value) {
                if (is_array($value)) {
                    // Si la el valor de la FK es un array (una tabala tiene 2 o más FK referenciando a una misma tabla)
                    foreach ($value as $v) {
                        $this->feedback['resource'] = $this->includeForeigns($this->feedback['resource'], $key, $v, $id);
                    }
                } else {
            ...
            foreach ($principal as $raw) {
                foreach ($foreignRaws['resource'] as $rawsForeign) {
                    if (strpos($id[0], $table) != false) {
                        ...
                        if (array_key_exists($key, $rawsForeign)) {
                            // Caso de FK y PK con mismos nombres
                            if ($raw[$key] == $rawsForeign[$key]) {
                                $raw[$key] = $rawsForeign;
                            }
                        } else {
                            // Caso de FK y PK con distintos nombres
                            // Todas las FK deben seguir nomenclatura-> xxx_NombreTabla_yyy siendo xxx el nombre de la PK que
                            // deben ser iguales en la tabla original y en la tabla referenciada
                            $pattern = '/_' . $table . '(.)*/';
                            $refactorKey = preg_replace($pattern, '', $key);
                            if ($raw[$key] == $rawsForeign[$refactorKey]) {
                                $raw[$key] = $rawsForeign;
                            }
                        }
                    }
    ...
    function constructWhereSearchLike($dataValuesQuery)
    {
        ...
        for ($i = 0; $i < count($this->dataQuery); $i++) {
            $infoQuery = $infoQuery . '(lower(' . $this->dataQuery[$i] . ') LIKE ? OR ' . $this->dataQuery[$i] . ' IS NULL) and ';
        }
        $infoQuery = substr($infoQuery, 0, -5);
        return $infoQuery;
    }
